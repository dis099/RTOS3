...
//void runperiodicevents(void); //EDN: function prototype of the sleep time decreamentor
...
int main_STEP4(void){
  ...
  //BSP_PeriodicTask_Init(&runperiodicevents, 1000, 6); //EDN: added to run periodic event where sleep time is decreamented
  OS_Launch(BSP_Clock_GetFreq()/1000);
  return 0;             // this never executes
}
...
int main_step5(void){
  ...
  OS_AddPeriodicEventThread(&TaskS, 10); //run every 10ms
  OS_AddPeriodicEventThread(&TaskU, 100); //run every 100ms
//  TExaS_Init(LOGICANALYZER, 1000); // initialize the Lab 3 grader
...
// functions in this file.
 ...
 //EDN: BEGINNING OF THE MAIN FUNCTION
 //int OS_AddPeriodicEventThreads(void(*thread)(void), uint32_t period); //EDN: Added it to differentiate the hardware timers
